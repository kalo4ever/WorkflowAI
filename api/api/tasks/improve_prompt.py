...
class ImprovePromptAgentInput(BaseModel):
    ...
    agent_run: AgentRun | None = Field(
        default=None,
        description="A user-evaluated run, obtained using the 'original_agent_config', to be used as the basis for improving the agent",
    )
...
@workflowai.agent(
    id="improve-prompt",
    model=Model.O1_MINI_2024_09_12,
)
async def run_improve_prompt_agent(_: ImprovePromptAgentInput) -> ImprovePromptAgentOutput:
    """Given an original agent config (prompt, input schema, output schema), and an example agent run (input, output, user evaluation), generate the following:
