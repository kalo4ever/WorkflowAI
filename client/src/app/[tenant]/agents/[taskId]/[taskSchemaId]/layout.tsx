...
import {
  detectPageIsRequiringTaskSchema,
  detectPageIsUsingNewDesign,
} from '@/lib/pageDetection';
...
export default function Layout({
  ...
  const {
    tenant,
    taskId = '_' as TaskID,
    taskSchemaId = '_' as TaskSchemaID,
  } = useTaskParams();
  ...
  const isUsingNewDesign = useMemo(
    () => detectPageIsUsingNewDesign(pathname),
    [pathname]
  );

  //TODO: In the future we should rearchitect a bit and remove the loader completly
  const isRequiringTaskSchema = useMemo(
    () => detectPageIsRequiringTaskSchema(pathname),
    [pathname]
  );
  ...
  if (taskSchema || !isRequiringTaskSchema) {
    return (
      <div
        className={cn(
          'w-full h-full',
          !isUsingNewDesign ? 'pt-[24px] pb-[16px]' : 'overflow-hidden'
        )}
      >
        {children}
        <DeployVersionModal />
