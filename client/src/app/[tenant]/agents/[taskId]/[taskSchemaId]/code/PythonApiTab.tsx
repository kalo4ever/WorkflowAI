...
export function PythonApiTab(props: PythonApiTabProps) {
  const { tenant, taskId, taskSchemaId, iteration, environment, taskRun, apiUrl, secondaryInput, semver } = props;
  ...
  const finalSnippet = useMemo(() => {
    ...
    if ('stream' in taskSnippet.run && 'run' in taskSnippet.run && 'common' in taskSnippet.run) {
  ...
  return (
    <div className='flex flex-col w-full h-full overflow-y-auto'>
      ...
      <CodeBlock
        ...
        snippet={finalSnippet.replace('__WORKFLOWAI_API_TOKEN__', 'Add your API key here')}
        showTopBorder={true}
      />
    </div>
  );
