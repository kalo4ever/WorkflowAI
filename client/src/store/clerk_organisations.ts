...
interface ClerkOrganizationState {
  ...
  fetchClerkOrganization: (organizationId: string) => Promise<OrganizationInformation | undefined>;
  fetchClerkOrganizations: (organizationIds: string[]) => Promise<void>;
}

export const useClerkOrganizationStore = create<ClerkOrganizationState>((set, get) => ({
  ...
  fetchClerkOrganization: async (organizationId: string) => {
    ...
    try {
      organization = await client.get<OrganizationInformation>(`/api/users/organizations/${organizationId}`);
      set(
        produce((state) => {
