...
export function AIModelScrollSelection(props: AIModelScrollSelectionProps) {
  ...
  const formattedModelOptions = useMemo(() => formatAIModels(modelOptions, sort), [modelOptions, sort]);

  const scrollTo = useCallback((index: number, instant: boolean): Promise<void> => {
  ...
  useEffect(() => {
    if (value) {
      const index = formattedModelOptions.findIndex((option) => option.value === value);
      scrollTo(index, true);
    }
      ...
      {formattedModelOptions.map((option, index) => (
        <div
          ...
          className={cx(
            ...
            value === option.value ? 'text-slate-700 bg-slate-200' : 'text-slate-600'
          )}
